...
DnnNode::DnnNode(const std::string &node_name, const std::string &namespace_)
{
  dnn_node_para_ptr_ = std::make_shared<DnnNodePara>();
  dnn_node_impl_ = std::make_shared<DnnNodeImpl>(dnn_node_para_ptr_);
}
...
int DnnNode::SetOutputParser() {
  ...
  std::cout<<"dnn, Run default SetOutPutParser ."<<std::endl;

  if (!dnn_node_para_ptr_) {
    //RCLCPP_ERROR(rclcpp::get_logger("dnn"), "Invalid node para!");
	  std::cout<<"dnn, Invalid node para !"<<std::endl;
    return -1;
  }
  ...
  if (!dnn_node_para_ptr_->output_parsers_.empty()) {
	...
	std::cout<<"dnn, Set output parser with dnn node para."<<std::endl;
    ...
    for (const auto &parser_pair : dnn_node_para_ptr_->output_parsers_) {
      ...
      if (!out_parser) {
        //RCLCPP_ERROR(rclcpp::get_logger("dnn"), "Invalid out_parser");
	      std::cout<<"dnn, Invalid out_parser"<<std::endl;
        return -1;
      }
      if (GetModel()->SetOutputParser(parser_pair.first, out_parser) != 0) {
        ...
        //             parser_pair.first);
	std::cout<<"dnn, Set output parser index %d fail!"<<parser_pair.first<<std::endl;
        return -1;
      }
      //RCLCPP_INFO(rclcpp::get_logger("dnn"),
      //            "SetOutputParser %d success",
      //            parser_pair.first);
      std::cout<<"dnn, etOutputParser "<<parser_pair.first<< "success"<<std::endl;
    }
  } else {
    ...
    //    "output tensors and should parse output_tensors in PostProcess.");
	  std::cout<<"dnn, Set output parser with default dnn node parser, you will get all"<<std::endl<<"output tensors and should parse output_tensors in PostProcess."<<std::endl;
...
int DnnNode::Init() {
  ...
  std::cout<<"dnn Node init ."<<std::endl;
  ...
  if (ret != 0) {
    //RCLCPP_ERROR(rclcpp::get_logger("dnn"), "Set node para failed!");
	  std::cout<<"dnn, Set node para failed!"<<std::endl;
    return ret;
  }
  ...
  if (ModelTaskType::InvalidType == dnn_node_para_ptr_->model_task_type) {
    //RCLCPP_ERROR(rclcpp::get_logger("dnn"), "Invalid model task type");
	  std::cout<<"dnn, Invalid model task type"<<std::endl;
    return -1;
  }
  ...
  if (!dnn_node_para_ptr_->bpu_core_ids.empty()) {
    if (dnn_node_para_ptr_->bpu_core_ids.size() !=
      ...
      //             dnn_node_para_ptr_->task_num);
      //
	    std::cout<<"dnn, DnnNodePara of bpu_core_ids size "<< dnn_node_para_ptr_->bpu_core_ids.size()<<"should be zero or equal with task_num "<<dnn_node_para_ptr_->task_num<<std::endl;
      return -1;
    }
    for (const auto &bpu_core_id : dnn_node_para_ptr_->bpu_core_ids) {
      if (bpu_core_id < BPUCoreIDType::BPU_CORE_ANY ||
        ...
        //             static_cast<int>(BPUCoreIDType::BPU_CORE_1));
	      std::cout<<"dnn, Invalid bpu_core_id "<<static_cast<int>(bpu_core_id)<<", which should be ["<<static_cast<int>(BPUCoreIDType::BPU_CORE_ANY)<<", "<<static_cast<int>(BPUCoreIDType::BPU_CORE_1)<<" ]"<<std::endl;
        return -1;
      }
    }
  }
  ...
  if (ret != 0) {
    ...
    std::cout<<"dnn Model init failed !"<<std::endl;
    return ret;
  }
  ...
  if (ret != 0) {
    ...
    std::cout<<"dnn Set output parser failed !"<<std::endl;
    return ret;
  }
  ...
  if (ret != 0) {
    ...
    std::cout<<"dnn Task init failed !"<<std::endl;
    return ret;
  }
...
int DnnNode::PostProcess(const std::shared_ptr<DnnNodeOutput> &output) {
  if (output) {
    ...
    std::cout<<"dnn Post process in dnn node"<<std::endl;
  }
  return 0;
}
