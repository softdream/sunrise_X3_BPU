...
int InitClassNum(const int &class_num) {
  if(class_num > 0){
    ...
    //             class_num);
	  std::cout<<"Yolo5_detection_parse, class_num = "<<class_num<<" is not allowed, only support class_num > 0"<<std::endl;
    return -1;
  }
  return 0;
}
  ...
  if (fi) {
    ...
    if(size != yolo5_config_.class_num){
      ...
      //           size, yolo5_config_.class_num);
	    std::cout<<"Yolo5_detection_parser, class_names length "<<size<<" is not equal to class_num "<<yolo5_config_.class_num<<std::endl;
      return -1;
    }
  } else {
    //RCLCPP_ERROR(rclcpp::get_logger("Yolo5_detection_parser"),
    ...
    //             cls_name_file.c_str());
	  std::cout<<"Yolo5_detection_parser, can not open cls name file : "<<cls_name_file.c_str()<<std::endl;
    return -1;
  }
  return 0;
}
...
int InitStrides(const std::vector<int> &strides, const int &model_output_count){
  ...
  if(size != model_output_count){
    ...
    //            size, model_output_count);
	  std::cout<<"Yolo5_detection_parser, strides size "<<size<<" is not equal to model_output_count "<<model_output_count<<std::endl;
    return -1;
  }
...
int InitAnchorsTables(const std::vector<std::vector<std::vector<double>>> &anchors_tables, 
  ...
  if(size != model_output_count){
    ...
    //            size, model_output_count);a
	  std::cout<<"Yolo5_detection_parser, anchors_tables size "<<size<<" is not equal to model_output_count "<<model_output_count<<std::endl;
    return -1;
  }
  ...
  for (size_t i = 0; i < anchors_tables.size(); i++){
    if(anchors_tables[i].size() != 3){
      ...
      //
	    std::cout<<"Yolo5_detection_parser, anchors_tables["<<i<<"] size is not equal to 3"<<std::endl;
      return -1;      
    }
	...
	//
	      std::cout<<"Yolo5_detection_parser, anchors_tables["<<i<<"]["<<j<<"] size is not equal to 2"<<std::endl;
        return -1;
      }
  ...
  if (document.HasMember("model_output_count")) {
    ...
    if (model_output_count <= 0){
      ...
      //        "model_output_count = %d <= 0 is not allowed", model_output_count);
	    std::cout<<"Yolo5_detection_parser, model_output_count = "<<model_output_count<<" <= 0 is not allowed !"<<std::endl;
      return -1;
    }
  }
...
void ParseTensor(std::shared_ptr<DNNTensor> tensor,
  ...
  if (ret != 0) {
    ...
    //             "get_tensor_hw failed");
	  std::cout<<"Yolo5_detection_parser, get_tensor_hw failed !"<<std::endl;
  }
...
int32_t Parse(
  ...
  if (ret != 0) {
    ...
    //            ret);
    //
	  std::cout<<"Yolo5_detection_parser, postprocess return error, code = "<<ret<<std::endl;
  }
  ...
  //RCLCPP_DEBUG(
	...
	std::cout<<"Yolo5_detection_parser "<<ss.str().c_str()<<std::endl;
  return ret;
}
